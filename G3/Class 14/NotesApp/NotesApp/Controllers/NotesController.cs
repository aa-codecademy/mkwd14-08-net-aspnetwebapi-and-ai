    ...
    public class NotesController : ControllerBase
    {
        ...
        public ActionResult<List<NoteDto>> GetAll() //the controller must return dtos instead of domain objects
        {
            ...
            catch (Exception ex)
            {
                ...
                string exceptionString = JsonConvert.SerializeObject(ex); //we want all the details (the whole expception object) logged - so we serialize it into json and log it in the file
        ...
        public ActionResult<NoteDto> GetById(int id) { //we return a DTO model
            ...
            try
            {
                Log.Information("Get by id was called");
        ...
        public IActionResult AddNote([FromBody] AddNoteDto note)
        {
            try
            {
               NoteDto newNote = _noteService.AddNote(note);
                Log.Information("Note was added");
                return Ok(newNote);
            }
        ...
        public IActionResult DeleteNoteById(int id)
        {
            try
            {
                ...
                // return Ok();
                Log.Information($"Note with id {id} was deleted");
                return NoContent();
            }
            catch(NullReferenceException ex)
            {
