...
This exercise is different. **You are not writing a new API — you are fixing one.**

In `LibraryApi/` there is a small, finished **Library API**: a Books controller,
...
- **Using the debugger** — breakpoints, stepping
...
1. Open `LibraryApi/LibraryApi.slnx` in Visual Studio.
...
```
   ...
   Keep this window open for the whole exercise. You will need it.
...
**This is the main skill this exercise trains.** Reading code and *thinking* about what it does is
...
Rule for this exercise: **before you blame the code, prove the row.** And after every fix,
...
This exercise is the one place where "just ask the AI" ruins the exercise completely. Pasting the
