...
namespace MoviesApi.Services.Implementations
{
    public class MovieService : IMovieService
    {
        ...
        public void UpdateMovie(UpdateMovieDto updateMovieDto, int userId)
        {
            ...
            if (!Enum.IsDefined(updateMovieDto.Genre))
            {
                throw new ArgumentException("Invalid value for genre");
            }
            ...
            //after we validated that the model that was sent as the updateDto is valid, we can now make a call to the db and make sure that the movie we are trying to update exists
            ...
            //map the new values to the existsing object from db
            ...
            movieDb.UserId = userId;

            if(updateMovieDto.Image != null)
            {
                using var memoryStream = new MemoryStream();
                updateMovieDto.Image.CopyTo(memoryStream);
                movieDb.Image = memoryStream.ToArray();
            }
